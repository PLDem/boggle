...
public class Board {
  ...
  /** Constructs the board, filling it
  ...
  public Queue<String> allWords() {
    ...
    HashMap foundWords = new HashMap();
  ...
  private void allWords(String sofar, int row, int col,
      boolean[][] used, HashMap foundWords) 
  {
    // Give up on words longer than 8 letters.
    if (sofar.length() > 8) return;
    if (row > 4 || col > 4 || row < 0 || col < 0) return;
    if(used[row][col] == true) return;
    
    //Add new character to word
    sofar += Character.toString(board[row][col]);
    
    //Check if this is an english word, if so add it to foundwords if it has not been found already
    if(englishWords.find(sofar))
    	if(!foundWords.find(sofar))
    		foundWords.insert(sofar);	
    
    //Set this square to true, signifying it has been used
    used[row][col] = true;
    ...
    //Once all spaces have been checked, back track by setting this square to false
    used[row][col] = false;
